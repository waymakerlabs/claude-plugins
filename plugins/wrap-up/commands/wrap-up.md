...
> 아래 항목 중 서로 파일·의존관계가 안 겹치는 것은 독립적이다 — Orca+codex 오케스트레이션(`orca-codex-delegate` v1.0.2+)으로 위임할 때 병렬 배치가 기본값이니 순차로 하나씩 시키지 않는다. 의존관계가 있는 항목은 "N번은 M번 완료 후"로 명시한다.

1. {다음 할 일 1} — 대상 `path/to/file` (독립 / 또는 "N번 완료 후")
2. {다음 할 일 2} — 대상 `path/to/file` (독립 / 또는 "N번 완료 후")
...
- ⚠️ **게이트는 코드 스타일을 일괄 변경하지 않습니다.** 포매터·린터 자동수정이 필요하면 *이번에 변경한 파일만* 대상으로 실행하고, 디렉터리나 `.`을 대상으로 돌리지 않습니다. 저장소가 애초에 그 포매터 기준으로 정렬돼 있지 않으면 디렉터리 1회 실행이 무관한 파일의 diff를 삼켜, 이번 변경이 무엇이었는지 알 수 없게 됩니다.
...
- **Orca+codex 오케스트레이션이 설정돼 있는 세션이면 이 리뷰를 `Skill(orca-codex-delegate:orca-codex-delegate)`(v1.0.2+) §4 크로스체크 절차로 수행하는 것이 기본값입니다.** 이번 세션에서 변경된 파일·영역이 여럿이고 서로 독립적이면(파일이 겹치지 않으면) 순차 1개씩이 아니라 같은 Run 안에서 `orca-review` worker 여러 개를 동시에 dispatch해 병렬로 리뷰합니다.
- 리뷰 도구와 실행 방식은 프로젝트/전역 설정을 따릅니다. 이 스킬은 특정 리뷰 도구를 가정하지 않으며, Orca가 없는 프로젝트에서는 설정된 다른 리뷰 도구를 그대로 씁니다.
- ⚠️ **리뷰를 별도 프로세스·워커·에이전트에 맡겼다면, 그것이 실제로 기동했고 결과를 수신했는지 확인합니다.** 실행 명령·프로세스 식별자·로그 또는 결과 경로를 남기고, 결과물이 비어 있거나 확인하지 못했으면 **`REVIEW-NOT-RUN`** 으로 보고한 뒤 리뷰 통과로 취급하지 않습니다. **오류가 없다는 사실을 성공으로 추론하지 않습니다** — 조용한 실패는 PASS가 아닙니다.
- 리뷰 결과가 "지적 없음"이면, 무엇을 검토 범위에 넣었고 무엇을 넣지 못했는지(한계)를 함께 적습니다. 범위 없는 "이상 없음"은 근거가 되지 않습니다.
