...
### Step 6: 검증 게이트 (Git 프로젝트만)

> ⚠️ **Git 프로젝트가 아닌 경우 이 단계를 건너뜁니다.**
> 💡 **목적**: 커밋·푸시 전에 깨진 코드가 원격/배포로 나가는 것을 차단합니다. "검증 없이 완료로 단언"하는 사고를 막는 게이트입니다.

커밋하기 전에, **저장소에 이미 정의되어 있는 검증만** 자동 감지해서 실행합니다. 없는 검사는 억지로 만들지 않고 건너뜁니다.

**6-0. 결정적 검사 자동 감지 & 실행:**

| 신호 | 실행할 것 |
|------|-----------|
| `package.json`의 scripts | 존재하는 것만 순서대로: `typecheck` → `lint` → `test` → `build` |
| `pyproject.toml` / `pytest.ini` / `tox.ini` | 설정돼 있으면 `pytest`, `ruff`, `mypy` |
| `Makefile`의 `verify`/`check`/`test` 타깃 | 해당 타깃 |
| 저장소 자체 검증 스크립트 (`scripts/verify.sh`, `scripts/check.sh` 등) | 해당 스크립트 |
| 회귀 가드 스크립트 (예: 산출물 byte-identical 비교) | 저장소에 있으면 실행 |

- 무엇을 실행했고 무엇을 (없어서) 건너뛰었는지 한 줄로 보고합니다.
- **하나라도 실패하면 즉시 중단**합니다. 버전 bump·커밋·푸시로 넘어가지 않습니다. 실패 내용을 daily log "실수 및 교훈"에 기록하고, 실패 로그와 함께 사용자에게 보고합니다.
- ⚠️ 검사를 돌리지 않았거나 실패한 상태에서 "통과했다 / 완료다"라고 단언하지 않습니다. 확인한 것만 보고합니다.

**6-1. 코드 리뷰 게이트 (설정돼 있는 경우만):**

- 프로젝트 또는 사용자 전역 규칙에 코드 리뷰 게이트(예: codex 리뷰)가 정의돼 있으면, working-tree diff를 대상으로 1회 실행하고 BLOCK/critical 지적을 먼저 해소한 뒤에만 다음 단계로 진행합니다.
- 리뷰 도구와 실행 방식은 프로젝트/전역 설정을 따릅니다. 이 스킬은 특정 리뷰 도구를 가정하지 않습니다.

> 💡 빠르고 결정적인 검사(6-0)는 여기서 수행합니다. 무인 자동 루프처럼 *매 커밋*을 보호해야 하면, 같은 검사를 별도 pre-commit/Stop 훅으로 추가하는 것을 권장합니다(스킬을 호출하지 않는 커밋도 막기 위함).

### Step 7: 프로젝트 버전 업데이트 (Git 프로젝트만)

> ⚠️ **Git 프로젝트가 아닌 경우 이 단계를 건너뜁니다.**

프로젝트 타입에 따라 패치 버전을 1 올립니다:

#### 7-1. Claude 플러그인 프로젝트
...
**주의사항**:
- Claude 플러그인 프로젝트는 7-1로 처리 (plugin.json 우선)
- 모노레포의 경우 루트 package.json만 업데이트

### Step 8: 관련 문서 업데이트 (Git 프로젝트만)
...
### Step 9: Git Commit & Push (Git 프로젝트만)

> ⚠️ **Git 프로젝트가 아닌 경우 이 단계를 건너뜁니다.**

**9-0. 안전 체크 (git add 전에 반드시 실행):**
...
9. **검증 게이트**(Git 프로젝트)는 커밋 전에 저장소에 정의된 검증(테스트·타입체크·회귀 가드·코드 리뷰 등)을 자동 감지해 먼저 실행하고, **실패 시 버전 bump·커밋·푸시를 중단**합니다. 없는 검사는 건너뜁니다. 검증하지 않은 상태로 "완료"를 단언하지 않습니다.
