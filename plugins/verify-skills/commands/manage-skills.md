...
등록된 스킬이 1개 이상인 경우, 각 스킬의 검증 파일을 읽고 다음에서 추가 파일 경로 패턴을 추출합니다:

1. **Related Files** 섹션 — 테이블을 파싱하여 파일 경로 및 glob 패턴 추출
2. **Workflow** 섹션 — grep/glob/read 명령어에서 파일 경로 추출

스킬 파일 탐색 순서:
1. `.claude/skills/verify-<name>/SKILL.md` (프로젝트 스킬)
2. 설치된 플러그인의 `commands/verify-<name>.md`
...
영향받은(AFFECTED) 각 스킬(매칭된 변경 파일이 있는 스킬)에 대해, 전체 스킬 파일을 읽고 다음을 점검합니다:
...
사용자가 업데이트를 승인한 각 스킬에 대해, 현재 스킬 파일을 읽고 대상 편집을 적용합니다:
...
4. **연관 파일 업데이트** — 새 스킬 생성 후 반드시 아래 파일들을 업데이트합니다:

   **4a. `verify-implementation` 스킬의 실행 대상 스킬 테이블 업데이트**

   **4b. `manage-skills` 스킬의 등록된 검증 스킬 테이블 업데이트**

   **4c. `CLAUDE.md`의 Skills 테이블 업데이트**

### Step 7: 검증

모든 편집 후:

1. 수정된 모든 스킬 파일을 다시 읽기
...
1. **Lock 파일 및 생성된 파일** — `package-lock.json`, `yarn.lock`, 자동 생성된 파일, 빌드 출력물은 스킬 커버리지가 불필요
2. **일회성 설정 변경** — 버전 범프, 린터/포매터 설정의 사소한 변경은 새 스킬이 불필요
...
7. **벤더/서드파티 코드** — `vendor/`, `node_modules/` 등 외부 코드는 외부 규칙을 따름
